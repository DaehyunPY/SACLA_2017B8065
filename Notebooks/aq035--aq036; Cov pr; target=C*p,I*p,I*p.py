...
def run(k0: str, k1: str, k2: str,
    ...
    with builder.getOrCreate() as spark:
        ...
        def hist(
            ...
            edges = {
                "pr": np.linspace(0, 1000, 1001),  # bin size: 1
            }
            h0pr, _ = np.histogram(
                [norm(r["inlab"]["hit0"]) for r in filtered],
                bins=edges["pr"],
            )
            h1pr, _ = np.histogram(
                [norm(r["inlab"]["hit1"]) for r in filtered],
                bins=edges["pr"],
            )
            h2pr, _ = np.histogram(
                [norm(r["inlab"]["hit2"]) for r in filtered],
                bins=edges["pr"],
            )
            return [xr.Dataset(
                {"n": len(filtered),
...
for i0, i1, i2 in [[1, 2, 1]]:
    ...
    filename = ("Data/Cov pr at the low int group; "
